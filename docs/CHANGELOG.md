...
- [修复] Web 股票自动补全补齐 JP/KR 市场建议徽标渲染，并放宽远端 stock-index 载荷校验的市场白名单以接纳 JP/KR；当前仅支持 `.T` / `.KS` / `.KQ` 后缀输入（未实现 bare JP/KR 代码后端自动映射、未改造历史/任务展示链路），不改动 provider/model/base URL 与运行时配置清理/迁移语义。
